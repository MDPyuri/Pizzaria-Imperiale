    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                error: 'Campos obrigatórios: data, valor, Usuario_idUsuario, Endereco_idEndereco, Pagamento_idpagamento',
            });
        }
        ...
        const pedido = await prisma.pedido.create({
            data: {
                ...
                Endereco_idEndereco: parseInt(Endereco_idEndereco),
                Pagamento_idpagamento: parseInt(Pagamento_idpagamento),
            },
        });
        res.json(pedido);
    } catch (error) {
...
pedidoRoutes.put('/atualizar/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { data, valor, Endereco_idEndereco, Pagamento_idpagamento } =
            req.body;
...
pedidoRoutes.put('/associar-endereco/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: 'Erro ao associar endereço ao pedido',
            details: error.message,
        });
    }
});
